...
def save_model(model, iter, name):
    torch.save(model.state_dict(), os.path.join(
        name, "iter_{}.pth.tar".format(iter)))
...
class ImageCompressor(nn.Module):
    ...
    def forward(self, input_image):
        quant_noise_feature = torch.zeros(input_image.size(0), self.out_channel_M, input_image.size(
            2) // 16, input_image.size(3) // 16).to(input_image.device)  # .cuda()
        quant_noise_z = torch.zeros(input_image.size(0), self.out_channel_N, input_image.size(
            2) // 64, input_image.size(3) // 64).to(input_image.device)  # .cuda()
        ...
        bpp_feature = total_bits_feature / \
            (batch_size * im_shape[2] * im_shape[3])
