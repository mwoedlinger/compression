...
# TODO: check which server the script is running and adjust accordingly
data_zoo = {
    'clic_mobile': {
        'train': Path('/data/databases/clic_mobile/train'),  # 1048
        'eval': Path('/data/databases/clic_mobile/valid'),  # 61
        'test': Path('/data/databases/clic_mobile/test')  # 178
    },
    'clic_professional': {
        'train': Path('/data/databases/clic_professional/train'),  # 585
        'eval': Path('/data/databases/clic_professional/eval'),  # 41
        'test': Path('/data/databases/clic_professional/test')  # 250
    },
    'clic': {
        'train': Path('/data/databases/clic/train'),  # 1633
        'eval': Path('/data/databases/clic/eval'),  # 102
        'test': Path('/data/databases/clic/test')  # 428
    },
    'kodak': {
        'eval': Path('/data/databases/kodak')  # 24
    },
    # Stereo
    'sidod': {
        'train': Path('/data/databases/sidod/train'),
        'eval': Path('/data/databases/sidod/eval'),
        'test': Path('/data/databases/sidod/test')
    }
}


class ImageDataloader(torch.utils.data.DataLoader):
    """Dataloader class for image compression datasets."""

    def __init__(self, **kwargs):
        dataset = ImageDataset(ds_name=kwargs['ds_name'], ds_type=kwargs['ds_type'],
                               transforms=kwargs['transforms'], debug=kwargs['debug'])
        self.files = dataset.files

        super().__init__(dataset=dataset,
                         batch_size=kwargs['batch_size'], num_workers=kwargs['num_workers'])

    def filename(self, idx: int) -> str:
        """Returns the filename of the correspoding idx
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, ds_name, ds_type, transforms, debug=False):
        """
        Args:
            ds_name (str): name of dataset (must be key of data_zoo)
            ds_type (str): train, eval or test
        ...
        self.data_dir = data_zoo[ds_name][ds_type]
    ...
    def __getitem__(self, idx):
        ...
        return self.transforms(image), idx

    def list_files(self) -> list:
        """Get list of all png image files in the data folder."""
        image_list = [file for file in self.data_dir.iterdir()
                      if file.suffix.lower() == '.png']

        if self.ds_name == 'sidod':
            image_list = [f for f in image_list if 'left.png' in str(f)]
...
def testKodak(step):
    with torch.no_grad():
        ...
        sumMsssim /= cnt
        sumMsssimDB /= cnt
        logger.info("Dataset Average result---Bpp:{:.6f}, PSNR:{:.6f}, MS-SSIM:{:.6f}, MS-SSIM-DB:{:.6f}".format(
...
if __name__ == "__main__":
    ...
    # TestKodakDataset(data_dir=args.val)
    test_dataset = ImageDataset(
        'sidod', 'eval', data_transforms_test, debug=False)
    test_loader = DataLoader(
        dataset=test_dataset, shuffle=False, batch_size=1, pin_memory=True, num_workers=1)
    if args.test:
        testKodak(global_step)
        exit(-1)
    ...
    train_dataset = ImageDataset(
        'sidod', 'train', data_transforms_train, debug=False)
